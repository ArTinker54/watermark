...
#:
#: На третьем месте стоит U+2061 FUNCTION APPLICATION — невидимый математический
#: оператор, в обычном тексте не встречающийся. Раньше там был U+200D ZERO WIDTH
#: JOINER, и это была ошибка: ZWJ — не «мусор редактора», а обязательная часть
#: составных эмодзи. Вырезая его, метка ломала 👩‍💻 и 👨‍👩‍👧 в каждой подписи, а
#: заодно любой текст с таким эмодзи выглядел для бота как помеченный.
_ALPHABET: Final[tuple[str, str, str, str]] = ("​", "‌", "⁡", "⁠")
_CODE: Final[dict[str, str]] = {char: f"{i:02b}" for i, char in enumerate(_ALPHABET)}

#: Прежний алфавит. Писать им перестали, а читать обязаны: материалы с такой
#: меткой уже разосланы, и терять по ним трассировку нельзя.
_LEGACY_ALPHABET: Final[tuple[str, str, str, str]] = (
    "​",
    "‌",
    "‍",
    "⁠",
)
_LEGACY_CODE: Final[dict[str, str]] = {
    char: f"{i:02b}" for i, char in enumerate(_LEGACY_ALPHABET)
}
...
#:
#: Последняя ветка — одинокий «<», за которым нет закрывающего. Без неё findall
#: такой символ просто пропускал, и при сборке текста он молча исчезал у ученика.
...
def has_marks(text: str) -> bool:
    ...
    Проверяется только нынешний алфавит — прежний отличался лишь ZWJ, который
    есть в каждом составном эмодзи, и обычное сообщение с 👨‍💻 срабатывало бы
    как помеченный текст.
    """
    return any(char in _CODE for char in text)


def strip_marks(text: str) -> str:
    """Убрать наши символы нулевой ширины — текст как его видит человек.

    ZWJ здесь намеренно не трогается, хотя прежний алфавит его использовал:
    он несёт смысл в эмодзи, и вырезать его — значит портить текст ученику.
    """
...
def extract(text: str) -> Payload | None:
    ...
    Сначала нынешним алфавитом, затем прежним: тексты, размеченные до отказа от
    ZWJ, обязаны читаться по-прежнему.
    """
    return _extract_with(text, _CODE) or _extract_with(text, _LEGACY_CODE)


def _extract_with(text: str, code: dict[str, str]) -> Payload | None:
    """Разбор одним алфавитом.
    ...
    назвать чужого.
    """
    found = [code[char] for char in text if char in code]
